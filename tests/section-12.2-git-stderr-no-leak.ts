...
async function main(): Promise<void> {
	await runSection(
		"12.2. extractGitGroundTruth does not leak git stderr to TUI",
		() => {
			...
			const tmpDir = fs.mkdtempSync(
				path.join(os.tmpdir(), "epistemic-git-stderr-"),
			);
			try {
				execSync("git init -b main", { cwd: tmpDir });
				execSync(
					'git -c user.name="Test" -c user.email="[email protected]" commit --allow-empty -m "init"',
					{ cwd: tmpDir },
				);
				...
				execSync(
					`git -c user.name="Test" -c user.email="[email protected]" commit -m "add lf file"`,
				...
				logPass(
					"extractGitGroundTruth captures git stderr instead of leaking to TUI",
				);
			} finally {
				try {
