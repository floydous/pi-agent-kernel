...
export function testPythonAstExtensions(): void {
	...
	try {
		...
		assertPass(
			"findSymbolReferences finds the two calculate_tax references",
			symbolRefs.length === 2 && symbolRefs.every((reference) => reference.filePath === "calculator.py"),
			{ symbolRefs },
		);
		...
		assertPass(
			"searchAstSymbols returns the calculate_tax method with an exact span",
			spanHits.length === 1 && spanHits[0].name === "calculate_tax" && spanHits[0].kind === "method" && spanHits[0].filePath === "calculator.py" && spanHits[0].line === 41 && spanHits[0].endLine >= spanHits[0].line,
			{ spanHits }
		);
