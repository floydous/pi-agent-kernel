...
async function main(): Promise<void> {
	await runSection("12. Epistemically-Grounded Chronological Compaction Engine", () => {
		...
		const mockBranchEntries = [
			{
				type: "message",
				message: { role: "user", content: "Implement Strategy 2 output clamping." },
			},
			{
				type: "message",
				message: {
					role: "assistant",
					content: [{ type: "toolCall", toolName: "edit", args: { path: "features/output_clamper.ts" } }],
				},
			},
			{
				type: "message",
				message: {
					...
					toolName: "edit",
					isError: false,
					content: [{ type: "text", text: "[EDIT SUCCESS] Applied via exact strategy." }],
				},
			},
			{
				type: "message",
		...
		assertPass(
			...
			trajectoryDigest.includes("<recent-turn-actions-digest>") &&
				trajectoryDigest.includes('[User Prompt]: "Implement Strategy 2 output clamping."') &&
				trajectoryDigest.includes('[Assistant Call]: edit({"path":"features/output_clamper.ts"})') &&
				trajectoryDigest.includes("[Tool Result bash]: SUCCESS -> ALL 11 TESTS PASSED"),
			{ trajectoryDigest }
		);
		...
		assertPass(
			...
			!prompt.includes("CRITICAL INSTRUCTIONS FOR CHRONOLOGICAL RECONCILIATION") &&
		...
		logPass("Chronological prompt topology verified (static instructions moved to system prompt)!");
	});
}
