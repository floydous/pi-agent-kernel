...
async function main(): Promise<void> {
	await runSection("22. Rust Full AST, Struct Bleed Defense, Variable Hover, Comment Filtering", () => {
...
`,
			...
			assertPass(
				...
				!!enumState && !enumState.signature.includes("impl State") && !enumState.signature.includes("record_success"),
				{ enumState }
			);
			assertPass(
			...
			logPass("Rust struct/enum snippet bleed prevention verified ('pub enum State')!");
			...
			logPass("Rust local variable hover verified ('(local variable) port: u16 = 8000')!");

			const rsStaticHover = extractLocalSymbolHover(rsStatePath, 0, 12, "ENABLED");
			assertPass(
			...
			const localMethodHover = extractLocalSymbolHover(rsStatePath, 15, 12, "record_success");
			assertPass(
			...
			const hasCommentRef = pickClientRefs.some((r) => r.lineText.startsWith("//"));
			assertPass(
			...
			logPass(`Comment word token filtering verified (${pickClientRefs.length} real code reference(s), 0 comment false positives)!`);
		} finally {
			ws.cleanup();
		}
	});
