...
async function main(): Promise<void> {
	await runSection("12.1. Compaction Prompt Caching Partitioning", () => {
		...
		logPass(
			"System prompt includes static instructions (cacheable across compactions)",
		);
		...
		logPass(
			"User message is free of static instructions (varies per compaction)",
		);
		...
		assertPass(
			...
			systemPrompt === roleOnly + ENHANCED_SUMMARIZATION_PROMPT,
			{
				...
				roleOnlyLen: roleOnly.length,
				staticPromptLen: ENHANCED_SUMMARIZATION_PROMPT.length,
			},
		);
		logPass(
			`System prompt is ${systemPrompt.length} chars; static instructions are ${ENHANCED_SUMMARIZATION_PROMPT.length} chars (the cacheable portion)`,
		);
	});
}
