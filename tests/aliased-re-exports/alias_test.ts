...
export async function testAliasedReExports(): Promise<void> {
	...
	try {
		...
		assertPass(
			"WebSocketClient alias resolves to the client definition",
			aliasHits.length === 2 && aliasHits.every((hit) => hit.name === "WebSocketClient") && aliasHits.some((hit) => hit.aliasedFrom?.originalName === "client"),
			{ aliasHits },
		);

		const origHits = searchAstSymbols(ws.tempDir, { name: "client", exactMatch: true });
		assertPass(
			"Original client definition is discoverable exactly once",
			origHits.length === 1 && origHits[0].name === "client" && origHits[0].kind === "class" && origHits[0].filePath === "src/webshocket/websocket.py",
			{ origHits },
		);
