...
async function main(): Promise<void> {
	await runSection(
		"20. Aliased Re-exports, Multi-line Signatures, Dotted Lookups, Import Hover",
		() => {
...
`,
				...
				logPass(
					"Multi-line aliased re-export resolution verified (WebSocketClient → client)!",
				);

				// Query dotted symbol webshocket.WebSocketClient
				const dottedHits = searchAstSymbols(ws.tempDir, {
					name: "webshocket.WebSocketClient",
				});
				assertPass(
				...
				logPass(
					"Dotted symbol lookup verified (webshocket.WebSocketClient resolved)!",
				);
				...
				logPass(
					"Multi-line signature extraction & multi-tier result ranking verified!",
				);
				...
				assertPass(
					"AST body preview reports truncation when bounded",
					largeBodyHits.some((hit) => hit.bodyTruncated === true),
					{ largeBodyHits },
				);

				// Test Hover on Import Line
				const initPyPath = path.join(srcDir, "__init__.py");
				const importHover = extractLocalSymbolHover(
					initPyPath,
					3,
					20,
					"rate_limit",
				...
				assertPass(
					...
					!!importHover &&
						importHover.includes("(imported symbol) rate_limit") &&
						!importHover.includes("(parameter)"),
					{ importHover },
				);
				...
				assertPass(
					...
					pathHits.length > 0 &&
						pathHits.every((hit) => hit.filePath.includes("src/webshocket")),
					{ pathHits },
				);
