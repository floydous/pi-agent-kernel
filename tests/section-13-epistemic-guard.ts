...
async function main(): Promise<void> {
	await runSection("13. Read-Before-Write Epistemic Guard Engine", () => {
		...
		try {
			...
			const uninspectedCheck = guard.checkReadPrecondition(ws.calculatorPath, "edit", TEST_SESSION);
			assertPass("Epistemic guard blocks edit on unread existing file", !uninspectedCheck.allowed, { uninspectedCheck });
			assertPass(
			...
			const newFileCheck = guard.checkReadPrecondition(nonExistingFilePath, "write", TEST_SESSION);
			assertPass("Epistemic guard allows write on brand-new file", newFileCheck.allowed, { newFileCheck });

			// Record file inspection via read
			guard.recordFileRead(ws.calculatorPath, TEST_SESSION);
			const inspectedCheck = guard.checkReadPrecondition(ws.calculatorPath, "edit", TEST_SESSION);
			assertPass("Epistemic guard allows edit after file was inspected", inspectedCheck.allowed, { inspectedCheck });
			...
			const extracted1 = extractInspectedFilesFromCommand(`cat "proxy_server.py" | grep -n "def"`, ws.tempDir);
			assertPass(
			...
			const extracted2 = extractInspectedFilesFromCommand(`head -n 50 calculator.py && grep "tax" proxy_server.py`, ws.tempDir);
			assertPass("Extract multiple files from compound bash command", extracted2.length >= 2, { extracted2 });

			// Verify EpistemicGuard allows edit after bash inspection
			guard.resetSession(TEST_SESSION);
			const preBashCheck = guard.checkReadPrecondition(proxyServerPath, "edit", TEST_SESSION);
			assertPass("Guard blocks edit before bash inspection", !preBashCheck.allowed, { preBashCheck });

			guard.recordCommandExecution(`cat proxy_server.py`, ws.tempDir, TEST_SESSION);
			const postBashCheck = guard.checkReadPrecondition(proxyServerPath, "edit", TEST_SESSION);
			assertPass("Guard allows edit after bash inspection", postBashCheck.allowed, { postBashCheck });

			// Per-session isolation
			guard.recordFileRead(ws.calculatorPath, "session_A");
			const otherSessionCheck = guard.checkReadPrecondition(ws.calculatorPath, "edit", "session_B");
			assertPass(
			...
			const crossCaseCheck = caseGuard.checkReadPrecondition(lowerCaseFile, "edit", "case_session");
			...
			logPass("Read-Before-Write epistemic guard & bash terminal inspection verified!");
		} finally {
			ws.cleanup();
		}
	});
