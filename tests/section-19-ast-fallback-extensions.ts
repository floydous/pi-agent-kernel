...
async function main(): Promise<void> {
	await runSection(
		"19. AST Fallback Extensions (Document Symbols, References, Hover)",
		() => {
			...
			try {
				...
				logPass(
					`extractDocumentSymbols verified (${docSyms.length} symbol(s) found)!`,
				);
				...
				logPass(
					`findSymbolReferences verified (${symbolRefs.length} reference(s) found across workspace)!`,
				);
