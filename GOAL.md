...
- **Status:** `PARTIAL / OPEN` (policy experiment recorded; implementation unchanged)
...
- **Experiment result (2026-08-30):** A bounded comparison used the real built-in bash tool and the registered edit tool. Current shape preflight allowed `cat target.ts`, `grep value target.ts`, and `cat target.ts > nul`; it rejected `grep -c`, `ls`, and other excluded modes. A result-aware simulation allowed ordinary output (`cat`, `grep`) but rejected redirected output (`cat target.ts > nul`), no-match/error output, and non-output modes. It also revealed that `cat target.ts | wc -l` still produces visible output while exposing only a line count, so output-presence alone is not sufficient evidence. A native-only simulation rejected every shell case. The real Pi hook ordering confirms result-aware recording cannot preserve same-batch shell-read/edit because `tool_call` runs before tool execution and sibling calls are prepared before results are available.
- **Decision:** Keep the current classified command-shape policy for now: it rejects clearly metadata/count/quiet/filename-only forms, preserves same-batch compatibility, and documents that it proves neither output exposure nor comprehension. A result-aware policy would need command semantics in addition to output inspection (for example, to reject `cat file | wc -l`), and would require changing same-batch behavior or tool scheduling. Stricter result-aware or native-only enforcement remains a deliberate future architectural change, not an unverified fix.
